...
projectList.get("/counts", async (req, res) => {
    ...
    try{
       ...
       const close= closedData.length;
       const running=runningData.length;
       const cancel=cancelledData.length;
       const closeure=closureDelay.length;

    //    console.log(totalData,closedData,runningData,cancelledData,closureDelay);
       res.send({totalData,close,running,cancel,closeure})
    }
    catch(err){
        console.log(err);
    }
...
projectList.get("/piecharts",async(req,res)=>{
    try{
        const data = await projectModel.find({});
       let Filters = data.filter((el)=>{
          return el.division==="Filters"
       })
       let Pumps = data.filter((el)=>{
        return el.division==="Pumps"
     })
     let Glass = data.filter((el)=>{
        return el.division==="Glass"
     })
     let water = data.filter((el)=>{
        return el.division==="Water Heaters"
     })
        
        let fil=Filters.length
        let pum=Pumps.length
        let gl=Glass.length
        let wat=water.length
        res.send({fil,pum,gl,wat})
    }
    catch(err){
        console.log(err);
    }
